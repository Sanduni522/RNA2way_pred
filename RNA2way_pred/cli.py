...
def prep_fasta_secstruct(pdb_file,path):
    ...
    ss1 = '(' + '.' * (len(seq1) - 2) + '('
    ss2 = ')' + '.' * (len(seq2) - 2) + ')'
    fasta = open(f'{path}/{pdb_file[:-4]}/{seq_low}.fasta', 'w')
    secstruct = open(f'{path}/{pdb_file[:-4]}/{seq_low}.secstruct', 'w')
...
def read_dssr_file(pdb_file,path):
    ...
    for i in range(x1):
        ...
        for j in range(34):
            ...
            names = sorted(glob.glob(f"{path}/{pdb_file[:-4]}/pdb/S_*"))
...
def compute_rmsd_from_coords(Q, P):
    ...
    Q -= Qc
    print(P,Q)
    rmsd = np.round(kabsch_rmsd(P, Q),2)
    return rmsd

def cal_rmsd(pdb_file,path):
    df = pd.read_csv(f"{path}/pdb_structures.csv") ### place where the software is installed
    ...
    all_coords_native = [get_coordinates_pdb(f"{path}/{pdb_file[:-4]}/{pdb_file}", [x1, x2, x3, x4])]
...
def add_RMSD(pdb_file,path):
    df_score = pd.read_csv(f'{path}/{pdb_file[:-4]}/pdb_structures_rms.csv')
    rmsd = df_score['rms']

    xls = pd.ExcelFile(f'{path}/{pdb_file[:-4]}/dssr_data.xls')
    for sheet_name in xls.sheet_names:
        df_final = pd.read_excel(f'{path}/{pdb_file[:-4]}/dssr_data.xls',sheet_name=sheet_name)
        df_final['RMSD'] = rmsd

        df_final.to_excel(f'{path}/{pdb_file[:-4]}/dssr_data_{sheet_name}_plus_rmsd.xlsx', sheet_name=sheet_name, index=False)
        print(df_final)
...
def SASA(pdb_file,path):
    ...
    for l in pos:
        df = pd.read_excel(f'{path}/{pdb_file[:-4]}/dssr_data_Sheet {l}_plus_rmsd.xlsx')  # name of the file that has list of models

        struct = df['name']
        nt_num = df['nt-num']
        index = len(df.index)

        path1 = f'{path}/{pdb_file[:-4]}/pdb'  # the models are inside this folder
...
def hbond(pdb_file,path):
    ...
    for n, l in zip(pos1, pos):
        ...
        name = glob.glob(f'{path}/{pdb_file[:-4]}/pdb/*')
        for fn in name:
            fn = fn.split('/')[-1]
            df_fn = pd.read_csv(f'{path}/{pdb_file[:-4]}/hbonds/{fn}_FARFAR-hbonds.csv')
            atom_1 = df_fn['Atom_1']
            atom_2 = df_fn['Atom_2']
            dist = df_fn['Distance']
            type = df_fn['Type']
            ...
            for j in range(index_fn):
                if (type[j] == 'p'):
                    ppdb = PandasPdb()
                    ppdb.read_pdb(f'{path}/{pdb_file[:-4]}/pdb/{fn}/{fn}.pdb')
                    ATOM = ppdb.df['ATOM']
                    atom = ATOM['atom_name']
                    resi_number = ATOM['residue_number']
                    length = len(ATOM.index)
...
def group_hbond(pdb_file,path):
    ...
    for n in range(j):
        ...
        df1_AC.to_csv(f"{path}/{pdb_file[:-4]}/N_{nuc}.csv")
        df2_AC.to_csv(f"{path}/{pdb_file[:-4]}/hbond_mean_{nuc}.csv")
...
def add_hbond(pdb_file,path):
    ...
    for l, k in zip(pos, pos1):
        sheet_num = k
        nt = l
        df = pd.read_csv(f"{path}/{pdb_file[:-4]}/structural_parameters_{sheet_num}.csv")
        df_index = df.index

        df_N = pd.read_csv(f"{path}/{pdb_file[:-4]}/N_{nt}.csv")
        df_mean = pd.read_csv(f"{path}/{pdb_file[:-4]}/hbond_mean_{nt}.csv")
        ...
        df.to_csv(f"{path}/{pdb_file[:-4]}/structural_parameters_{sheet_num}.csv", index=False)

def get_structures_less_than_RMSD(pdb_file,path):
    path1 = f'{path}/{pdb_file[:-4]}/structural_parameters_*.csv'
    filename = sorted(glob.glob(path1))

    for n, fn in enumerate(filename):
        df_final = pd.read_csv(fn)
        df = df_final[df_final['RMSD'] < 2.5]
        df.to_csv(f'{path}/{pdb_file[:-4]}/less_than_rmsd_{n + 1}.csv', index=False)
        print("DONE")
...
def concat_average_files(path):
    ...
    df_final.to_csv(f'{path}/results/Averages_concat_file.csv',index=False)
...
def add_DMS(path):
    df = pd.read_csv(f'{path}/results/Averages_concat_file.csv')
    res = string_pos(df)
    ...
    df.to_csv(f'{path}/results/Average_concat_file_with_DMS.csv', index=False)
...
def cli():
    ...
    test_df = pd.read_csv(f'{path}/results/Average_concat_file_with_DMS.csv')
    ...
    df_pred.to_csv(f'{path}/results/Predictions.csv', index=False)
    ...
    plt.plot(target_np, yfit, 'r-', lw=2, alpha=0.6, label=f'y = {np.round(yfit[0],3)} + {np.round(yfit[1],3)}x')
