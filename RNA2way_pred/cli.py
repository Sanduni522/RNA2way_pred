...
def run_dssr(pdb_file,path):
    ...
    for model_pdb in filenames:
        ...
        subprocess.call(['mv', f'{path}/{pdb_file[:-4]}/pdb/{model_pdb}', f'{path}/{pdb_file[:-4]}/pdb/{model_pdb[:-4]}/'])
        subprocess.call(['x3dna-dssr', f'-i={path}/{pdb_file[:-4]}/pdb/{model_pdb[:-4]}/{model_pdb}', f'-o={path}/{pdb_file[:-4]}/pdb/{model_pdb[:-4]}/{model_pdb[:-4]}.out'])
        subprocess.call(['mv', f'{path}/{pdb_file[:-4]}/pdb/dssr-torsions.txt', f'{path}/{pdb_file[:-4]}/pdb/{model_pdb[:-4]}/'])
...
def cal_hbond(pdb_file,path):
    ...
    filenames = sorted(glob.glob(f'{path}/{pdb_file[:-4]}/pdb/*/*.pdb'))
    for model_pdb in filenames:
        subprocess.call(['x3dna-dssr', f'-i={model_pdb}/{model_pdb[:-4]}.pdb', '--get-hbond', f'-o={model_pdb}/{model_pdb[:-4]}_FARFAR-hbonds.txt'])
